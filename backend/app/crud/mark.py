...
class crudMark:
    ...
    async def update_mark(new_mark_data: MarkUpdate,
                          db: Session):
        mark = await crudMark._get_mark_from_db(new_mark_data.exam_id, new_mark_data.student_id, db)
    ...
    async def get_mark(exam_id: int,
                       student_id: int,
                       db: Session):
        mark = await crudMark._get_mark_from_db(exam_id, student_id, db)
    ...
    async def get_exam_marks(exam_id: int,
                             db: Session):
        marks = await crudMark._get_exam_marks_from_db(exam_id, db)

        return list(MarkOut.model_validate(mark) for mark in marks)


    @staticmethod
    async def _get_exam_marks_from_db(exam_id,
                                     db: Session):
    ...
    @staticmethod
    async def _get_mark_from_db(exam_id: int,
                               student_id: int,
                               db: Session):
